...
On Android Chrome, an installed PWA can appear in the system share list (`share_target` in `frontend/public/manifest.webmanifest`, with an absolute `https://neshanak.ca/save` action). Chrome often puts the page address in the `text` query param rather than `url`; `/save` accepts both. After a successful save, `/save` replaces itself with `save-done.html` before any `window.close()`, because close on a share-target window returns focus to the previous app and freezes the form. `save-done.html` then closes itself. A home-screen shortcut is not a PWA and does not appear in Share. A small service worker (`/sw.js`) is registered as soon as the app loads so Chrome will treat Neshanak as installable. The install prompt is stored on `window` immediately, because Chrome fires it once before Settings is open. The worker passes requests through to the network and does not cache pages. This does not work on iPhone.
