...
On Android Chrome, an installed PWA can appear in the system share list (`share_target` in `frontend/public/manifest.webmanifest`, with an absolute `https://neshanak.ca/save` action). Chrome often puts the page address in the `text` query param rather than `url`; `/save` accepts both. After a successful save, `/save` closes itself and, on a share-target window, also goes back and replaces the form with `save-done.html` so the WebAPK task does not stay on the form. A home-screen shortcut is not a PWA and does not appear in Share. A small service worker (`/sw.js`) is registered as soon as the app loads so Chrome will treat Neshanak as installable. The install prompt is stored on `window` immediately, because Chrome fires it once before Settings is open. The worker passes requests through to the network and does not cache pages. This does not work on iPhone.
