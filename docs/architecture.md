...
On Android Chrome, an installed PWA can appear in the system share list (`share_target` in `frontend/public/manifest.webmanifest`). Chrome often puts the page address in the `text` query param rather than `url`; `/save` accepts both. A small service worker (`/sw.js`) exists so Chrome will offer Add to Home screen. It does not cache pages. This does not work on iPhone.
