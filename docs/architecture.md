...
On Android Chrome, an installed PWA can appear in the system share list (`share_target` in `frontend/public/manifest.webmanifest`). Chrome often puts the page address in the `text` query param rather than `url`; `/save` accepts both. A small service worker (`/sw.js`) is registered as soon as the app loads so Chrome will treat Neshanak as installable. It passes requests through to the network and does not cache pages. This does not work on iPhone.
