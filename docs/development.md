...
3. Drag the **Save To Neshanak** button from Settings onto that bar. The control includes the terracotta mark so the bar bookmark is not a blank label. Do not only click it on Settings — that does nothing on purpose, so the page does not navigate away while you drag.
